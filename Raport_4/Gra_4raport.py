...
life_img = pygame.image.load("zycko3.png")
last_life = pygame.image.load("ostatnie.png")
...
pygame.init()
x0 = pygame.mixer.Sound('muzyka1_wiesiek (online-audio-converter.com).wav')
x0.set_volume(0.2)
x1 = pygame.mixer.Sound('on&on (online-audio-converter.com).wav')
x1.set_volume(0.2)
x2 =pygame.mixer.Sound("Ican'tstop (online-audio-converter.com).wav")
x2.set_volume(0.2)
timer_sec = 5
objectHit = False
wallHit = False
...
def crash():
    global wallHit, timer_sec
    wallHit = True
    pygame.mixer.music.set_volume(0.2)
    pygame.mixer.music.load('punch_or_whack_-Vladimir-403040765.wav')
...
def game():
    global x, y, move_x, move_y, score, wallHit, objectHit, life

    timer_font = pygame.font.Font("Gotu-Regular.ttf", 60)
    timer_text = timer_font.render("5", True, (255, 0, 0))
    ...
    while not gameExit:
        for event in pygame.event.get():
            ...
            if event.type == timer:
                if timer_sec > 0:
                    timer_sec -= 1
                    timer_text = timer_font.render("%2d" % timer_sec, True, (255, 0, 0))
        ...
        score_text(gameDisplay, str('Score:'), 4, 425, 10)
        score_text(gameDisplay, str(score), 4, 555, 10)
        score_text(gameDisplay, str("; Time:"),4, 666, 10 )
        ...
        if timer_sec == 0:
            ...
            crash2()
            timer_sec = 6
            gameDisplay.blit(timer_text, (770, 15))
            if life == 0:
                gameExit = True
                life = 3
        elif objectHit == True:
        ...
        if life == 3:
            gameDisplay.blit(life_img, (display_width - 50, 10))
            gameDisplay.blit(life_img, (display_width - 90, 10))
            gameDisplay.blit(life_img, (display_width - 130, 10))
        elif life == 2:
            gameDisplay.blit(life_img, (display_width - 50, 10))
            gameDisplay.blit(life_img, (display_width - 90, 10))
        elif life == 1:
            gameDisplay.blit(last_life, (display_width - 50, 10))
